...
def copy_and_distribute_photos():
    ...
    success_count = 0
    skip_count = 0 # 最終結果で重複スキップ数を表示するため
    ...
    for i, file_path in enumerate(files):
        ...
        # dtから年月（YYYYMM）を取得してディレクトリ名に設定
        sub_dir_name = dt.strftime("%Y%m")
    ...
    print(f"処理完了: {success_count} 件のファイルを整理し、{skip_count} 件の重複をスキップしました。")
